...
class SRBM(Equation):
    ...
    def w_tf(self, x): #num_sample * 1
        return tf.reduce_sum(x, 1, keepdims=True)
    def V_true(self, x):
        return tf.reduce_sum( -tf.math.square(x) / 2 / self.mu[0], 1, keepdims = True)
    def V_grad_true(self, x):
        return - x / self.mu[0]

    def c_true(self):
        return  -1 / self.mu[0] * self.a * (1 + (self.dim - 2) * self.r - (self.dim - 1) * self.rho * self.r) / 2 / (1 - self.r) * self.dim 
